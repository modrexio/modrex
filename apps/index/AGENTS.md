...
state; `downloadable_observations` and `downloadable_entries` preserve immutable release and
hash relationships, including successful empty outcomes. A hosted file's storage object,
size and type define its remote revision; advertised version/URL churn alone does not force
...
**An off-site link is optional content, and failures belong to that downloadable rather than its parent mod.** `UnusableDownloadError` settles the link as successfully examined but unindexable; `TransientFetchError` records attempts plus a bounded next retry. Successful empty outcomes are also durable. Sibling files/links that already succeeded stay complete, so one failing host does not make Modrex redownload them. The parent discovery check can advance because the retry remains independently selectable. Postgres errors, non-404 ModWorkshop discovery errors and unknown extraction errors still fail the run; expected unusable/transient archive outcomes do not. `test-process-failure.ts`, `test-marker-archive.ts` and `test-postgres-architecture.ts` hold those contracts.
